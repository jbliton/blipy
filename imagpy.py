...
def fit_gmm(data, file_path = None, file_name = None, num_components = None, ln_transform = True, columns = None, labels = None):
    ...
    if columns: 
        for column in columns:
            ...
            data_score = gmm.score_samples(data[f'{column}'].to_numpy().reshape(-1,1))
            model_score = gmm.score(data[f'{column}'].to_numpy().reshape(-1,1))
            data[f'{column}_log-likelihood'] = data_score
            data[f'{column}_model_score'] = model_score
            ...
            if n_components ==3:
                ...
                # plt.show()
            if n_components ==2:
                high_sort_pos = data_sort[1]
                low_sort_pos = data_sort[0]
                
                label_map = {
                high_sort_pos:'positive',
                low_sort_pos:'neg',
                }
                
                print('Total cells in condition :',len(data[f'{column}']))
                d0 = pd.DataFrame(data[data[f'{column}_label']== 0], columns = ['predicted_label',f'{column}']) 
                print(f'Number cells in {column}-neg :',len(d0))
                d1 = pd.DataFrame(data[data[f'{column}_label']== 1], columns = ['predicted label',f'{column}']) 
                print(f'Number cells in {column}-pos :',len(d1))
                # plt.hist(d0, density = True, bins = 50)
                # plt.show()
                # plt.hist(d1, density = True, bins = 50)
                # plt.show()
            elif n_components ==1:
                high_sort_pos = data_sort[0]
